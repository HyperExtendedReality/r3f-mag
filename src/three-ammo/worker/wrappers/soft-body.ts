...
export class SoftBody {
  ...
  constructor(
    ...
    if (physicsAttributes) {
      this.numVerts = physicsAttributes.vertexFloatArray.length / 3;
    } else {
      ...
      case SoftBodyType.TRIMESH:
        if (physicsAttributes) {
          this.physicsBody = world.softBodyHelpers.CreateFromTriMesh(
            ...
            physicsAttributes.vertexFloatArray as any,
            physicsAttributes.indexIntArray as any,
            physicsAttributes.indexIntArray.length / 3,
  ...
  copyStateToBuffer() {
    ...
    if (this.mapping) {
      const visualVerts = this.buffers.vertexFloatArray.length / 3;
      for (let visualIndex = 0; visualIndex < visualVerts; visualIndex++) {
        const physicsIndex = this.mapping[visualIndex];
        const node = nodes.at(physicsIndex);
        const nodePos = node.get_m_x();
        const bufferIndex = visualIndex * 3;

  ...
  useFrame(() => {
    ...
    if (
      ...
      if (lastSubstep !== physicsPerformanceInfoRef.current.substepCounter) {
        const parentInverseCache = new Map<number, Matrix4>();
        ...
        for (let i = 0; i < uuids.length; i++) {
          ...
          if (type === BodyType.DYNAMIC) {
            ...
            let inv = parentInverseCache.get(parent.id);
            if (!inv) {
              inv = new Matrix4().copy(parent.matrixWorld).invert();
              parentInverseCache.set(parent.id, inv);
            }

            transform.multiplyMatrices(inv, matrix);
